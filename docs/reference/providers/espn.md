...
- **NCAA scoreboards**: NCAA leagues whose ungrouped ESPN scoreboard omits subdivisions fetch and merge ESPN's division groups. This includes FBS, FCS, lower-division, and cross-division fixtures when ESPN lists them. Teamarr cannot match fixtures ESPN does not expose in a public scoreboard group. College football and both college basketballs can drop a division from the fetch entirely — see [Divisions](../../guide/channels/output.md#available-overrides) under Per-League Channel Config. Cross-division games are filed under both divisions by ESPN, so they survive either selection; women's volleyball and women's lacrosse also pull one conference group each (United Athletic, Mid-American) that ESPN leaves out of the division slate, and those are always fetched.
