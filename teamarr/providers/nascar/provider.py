...
class NASCARProvider(SportsProvider):
    ...
    it in memory. get_events() filters to races whose session window covers
    the requested date, matching StaticCalendarProvider's contract so the
    racing matcher's _covers_date check works correctly.
    """
    ...
    def _load(self) -> None:
        ...
        for league, (suffix, series_key) in _LEAGUE_CONFIG.items():
            url = f"{_BASE_URL}/{self._year}/{suffix}"
            if url not in fetched:
                fetched[url] = self._fetch(url)

            raw = fetched[url]
            if raw is None:
                logger.warning("[NASCAR] No data for %s (year=%d)", league, self._year)
                self._events_by_league[league] = []
                continue
            ...
            logger.info("[NASCAR] Loaded %d races for %s (%d)", len(events), league, self._year)
