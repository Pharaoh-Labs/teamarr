...
**Push-button alternative (`Cut Release` workflow, #281):** Actions tab → "Cut Release" → Run workflow from `dev`, pick patch/minor/major (or an explicit version). CI re-runs the gates, bumps `pyproject.toml` + `uv.lock`, pushes the bump to dev, fast-forwards main, pushes the tag, opens a **draft** release, and dispatches the Docker publishes for `main`, `dev` and the tag (explicit dispatch — `GITHUB_TOKEN` pushes don't trigger workflows). Steps 7–9 stay manual: write curated notes on the draft and publish it, generate the Discord changelog, update `plans/STATUS.md`. After a Cut Release run, `git pull` locally — the bump commit lands on dev via the bot.
...
All `update_channel` calls go through `_safe_update_channel`, which checks `OperationResult.success` before persisting to local DB. On API failure, the DB stays unchanged so drift is re-detected on the next generation run. Profile sync also compares against Dispatcharr's actual state (`current_channel.channel_profile_ids`) for self-healing. Profile add/remove sets are diffed against that actual membership (or the catalog, for a stored `[0]`), never the stored list alone, and `0` is never queued as a profile — the ALL-profiles sentinel is not a profile (#894). New ids persist only once their bulk updates land (`_pending_profile_db_writes`). `_validate_profile_ids` re-reads the profile catalog once before calling an id stale, because the dynamic resolver creates `{sport}`/`{league}` profiles mid-run. Reconciliation (`reconciliation.py`) detects stream and profile drift as additional drift fields.
...
- **An alias widens an exact identity, never narrows it (#874).** `TEAM_ALIASES` used to be consulted only when the text had no exact hit, so a bare label that is one sport's full name and only an *alias* of the other sport's team had no route across: "Ireland" is the rugby side's full name while ESPN spells the soccer side "Republic of Ireland" (short name "Rep Ireland"), and `Kosovo vs Ireland` vetoed the very Nations League fixture it named (prod runs 125292/125293, 2 of 16 streams). The exact branch of `resolve` now also carries the exact identities of every alias rewrite; the rugby reading stays and stays exact. Same shape for `holland` → Netherlands. The alias table is the only place such spellings live — the country resolver only maps translations of ESPN's own name.
